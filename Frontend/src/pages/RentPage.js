...
function Rent() {
  ...
  useEffect(() => {
    axios
      .get(`http://127.0.0.1:8000/api/boats/${params.id}`)
      .then((response) => {
        setBoat(response.data.data[0]);
        setLoading(false);
      });
  }, [params.id]);
  ...
  const calculatePrice = () => {
    ...
    if (rentalDate.current.value && returnDate.current.value) {
      if (rental_date < now || return_date < now) {
        ...
        const price = (rentDuration / (1000 * 60 * 60 * 24)) * boat.price;
        setTotalPrice(price);
      }
    }
  };
      ...
      <Center h={"100vh"} m={["5%", "10%", "12%", "13%", "0%"]}>
        ...
        >
          <Box w={{ base: "100%", lg: "50%" }}>
            <Image src={boat.photo2} objectFit="cover" h={"full"}></Image>
          </Box>
          <Box w={{ base: "100%", lg: "50%" }} p={"5%"} bg={"white"} h={"full"}>
            <VStack alignItems={"center"} spacing={"3"}>
              <Heading fontWeight={"500"}>{boat.brand}</Heading>
              ...
              <SimpleGrid w={"full"} columns={3} py={3} textAlign="center">
                <GridItem>
                  ...
                  <Text fontWeight="600" color="gray.600">
                    {boat.fuel_type === "petrol" || boat.fuel_type === "diesel"
                      ? t(`boatCard.${boat.fuel_type.toLowerCase()}`)
                      : boat.fuel_type}
                  </Text>
                </GridItem>
                <GridItem>
              ...
              <Button onClick={rentABoat} w={"full"}>
                {t("boatCard.confirmRent")}
              </Button>
            </VStack>
          </Box>
        </Stack>
